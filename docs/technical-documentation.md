...
if the validation fails, an error message is shown. If validation succeeds, a success message is shown.
...
### `greetingMessage()`
displays a greeting message based in the time of day.
         
### `projectSearch()`
Filters the project cards based on the keywords entered in the search bar by the user.

### `formValidation()`
Validates the contact form and shows either a success message or error message.  
